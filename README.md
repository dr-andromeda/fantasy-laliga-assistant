...
Data comes from LaLiga Fantasy's public endpoint (`--source api`); if it's
unreachable the tool falls back to a local CSV. A **fictional** sample dataset
ships in `data/sample/` — invented clubs, players and prices, regenerated by
`data/sample/generate.py` — so the commands, the tests and the examples all run
with no network. It is not real data; use `--source api` for that.
